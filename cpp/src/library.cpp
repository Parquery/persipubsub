...
persipubsub::QueueData persipubsub::lookup_queue_data(const lmdb::env &env) {
    ...
    boost::split(subscriber_ids, subscriber_ids_str, boost::is_any_of(" "),
                 boost::token_compress_on);

    return QueueData(msg_timeout_secs, max_msgs_num, hwm_db_size_bytes,
                     strategy, subscriber_ids);
}