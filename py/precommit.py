...
def main() -> int:
    ...
    if overwrite:
        subprocess.check_call([
            ...
            "persipubsub", "setup.py", "precommit.py"
        ],
                              cwd=repo_root.as_posix())
    else:
        subprocess.check_call([
            "yapf", "--diff", "--style=style.yapf", "--recursive", "tests",
            "persipubsub", "bin", "setup.py", "precommit.py"
        ],
                              cwd=repo_root.as_posix())
    ...
    if overwrite:
        cmd = [
            ...
            "--dont-skip", "__init__.py", "--project", "persipubsub"
        ]
    ...
    subprocess.check_call([
        "coverage", "run", "--source", "persipubsub", "-m", "unittest",
        "discover", "tests"
    ],
                          cwd=repo_root.as_posix(),
                          env=env)
    ...
    for pth in (repo_root / "persipubsub").glob("**/*.py"):
